...
public class ListOfEntitiesHandler implements Handler {
    ...
    public Map<ResulType, Result> handle(LaunchParam param) {
        ...
        List<String> additional = jiraTaskService.additionalEnrichment(param.getTasks());
        ...
        try {
            ...
            result.get(ResulType.ERROR)
                    .add(Collections.singletonMap(MESSAGE, "error email notification: " + e.getMessage()));
            log.error(e.getMessage(), e);
        }
